...
export const ALGORITHMS: Algorithm[] = [
  ...
  {
    id: "library", name: "図書館ソート", english: "LIBRARY SORT", character: "ライブラ・シェルフ",
    category: "unique", icon: 26, accent: "#a9825e", tagline: "本棚にすき間を残して、さっと差し込む。",
    description: "挿入ソートの改良版。棚に空きスロットを保ちながら本を並べる司書のように、値をすき間へ差し込みます。",
    how: "ずらす手間をすき間が吸収し、埋まったら棚全体を等間隔に並べ直します。ギャップ挿入ソートとも呼ばれる発想です。",
    average: "O(n log n)", worst: "O(n²)", memory: "O(n)", stable: true,
  },
  {
    id: "patience", name: "ペイシェンスソート", english: "PATIENCE SORT", character: "ペイシェ・パイル",
    category: "fast", icon: 27, accent: "#8b6fa3", tagline: "トランプの山に配って、小さい順に回収。",
    description: "一人遊びトランプ由来。各カードを置ける最も左の山へ積み、最後に山の上端から最小値を拾い集めます。",
    how: "山の数は最長増加部分列の長さと一致します。配る段階は二分探索で速く、回収は山どうしの小さい値比べで進みます。",
    average: "O(n log n)", worst: "O(n²)", memory: "O(n)", stable: false,
  },
  {
    id: "dualPivot", name: "デュアルピボットクイックソート", english: "DUAL-PIVOT QUICKSORT", character: "デュエラ・スラッシュ",
    category: "fast", icon: 28, accent: "#c46584", tagline: "二本のピボットで、三つの区間へ切り分ける。",
    description: "小さめと大きめの2つの基準値を選び、配列を三分割してそれぞれの区間を再帰的に整列します。",
    how: "Java の Arrays.sort に採用された実戦派。分割の往復が減り、現実のデータで通常のクイックソートを上回ることがあります。",
    average: "O(n log n)", worst: "O(n²)", memory: "O(log n)", stable: false,
  },
  {
    id: "smooth", name: "スムースソート", english: "SMOOTHSORT", character: "スムーサ・レオナ",
    category: "fast", icon: 29, accent: "#6f9d8a", tagline: "レオナルド数の山々で、整い具合に寄り添う。",
    description: "ダイクストラ考案のヒープソート改良版。レオナルド数サイズのヒープの連なりを築き、右端から最大値を確定します。",
    how: "ほぼ整った入力なら O(n) に近づく適応型。山の合体と分裂が波のように起こる様子が見どころです。",
    average: "O(n log n)", worst: "O(n log n)", memory: "O(1)", stable: false,
  },
  {
    id: "sleep", name: "スリープソート", english: "SLEEP SORT", character: "ネムネ・アラーム",
    category: "unique", icon: 30, accent: "#7a86c2", tagline: "値の分だけ眠って、起きた順に並ぶ。",
    description: "各値が「自分の数字ぶん眠る」ネタ系ソート。目覚まし時計が鳴った順に列へ加わると、なぜか小さい順になっています。",
    how: "本来は並行処理のタイマー任せ。ここでは時計の刻みをシミュレートして再現します。比較は一度も行いません。",
    average: "O(n + max)", worst: "O(n + max)", memory: "O(n)", stable: true,
  },
  {
    id: "flash", name: "フラッシュソート", english: "FLASHSORT", character: "フラッシャ・スパーク",
    category: "linear", icon: 31, accent: "#d9a441", tagline: "分布を読んで、ひとっ飛びに定位置へ。",
    description: "値の分布から行き先のクラスを予測し、サイクル状の書き込みで一気におおよその位置へ運びます。",
    how: "分類、巡回移動、仕上げの挿入ソートの三段構え。均等に散らばるデータなら移動がほぼ一巡で済みます。",
    average: "O(n + m)", worst: "O(n²)", memory: "O(m)", stable: false,
  },
  {
    id: "americanFlag", name: "アメリカンフラッグソート", english: "AMERICAN FLAG SORT", character: "フラッガ・スター",
    category: "linear", icon: 32, accent: "#b55b5b", tagline: "大きい桁から、その場で旗のように区分け。",
    description: "基数ソートの上位桁版を追加の配列なしで行い、同じ桁の値ごとの区画へ交換だけで運び込みます。",
    how: "国旗の縞のように区画が並ぶのが名前の由来。各区画の中は、ひとつ下の桁で再帰的に整えます。",
    average: "O(d·n)", worst: "O(d·n)", memory: "O(k)", stable: false,
  },
  {
    id: "mergeInsertion", name: "マージ挿入ソート", english: "MERGE-INSERTION SORT", character: "メリサ・フォード",
    category: "fast", icon: 33, accent: "#5f8fa5", tagline: "ペア戦と絶妙な挿入順で、比較を節約。",
    description: "フォード＆ジョンソン法。ペアで勝ち抜きを行い、負けた側をヤコブスタール数が定める順番で二分挿入します。",
    how: "比較回数が理論下限に迫るのが売り。比較がとても高価な場面のために設計された、職人肌のソートです。",
    average: "O(n log n)", worst: "O(n log n)", memory: "O(n)", stable: false,
  },
  {
    id: "slow", name: "スローソート", english: "SLOW SORT", character: "ユルリ・スロウ",
    category: "unique", icon: 34, accent: "#9b8d7f", tagline: "増やして、降参。わざと遠回りの再帰。",
    description: "分割統治のパロディ「乗算と降伏」の代表作。左右を整えて最大値を末尾へ送ったあと、残り全体をもう一度整え直します。",
    how: "ストゥージソートを超える遅さで、多項式時間にも収まりません。じっくり味わうため要素数は40までです。",
    average: "n^(log n) 級", worst: "n^(log n) 級", memory: "O(log n)", stable: false, maxN: 40,
  },
];
