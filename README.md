...
- **リアルタイム可視化** — 比較中・交換中・配置中の要素を色分けし、一時的に取り出した棒も重複なく表現。いま行っている操作は凡例側でも点灯し、色だけに頼らず状態が分かる
- **マージ用の作業領域** — 左右の一時配列、比較中の値、結合先を処理に合わせて表示
- **非比較系の補助領域** — カウント表、バケット、出力配列、ビーズの重力ラック、図書館ソートのすき間つき本棚を処理に合わせて表示
- **自由な実験** — 要素数 `n`（最大256）と再生速度をその場で変更。要素が増えても棒が潰れず、密度に応じて自動でくっきり表示
...
> [!TIP]
> 同じ並びで別のアルゴリズムへ切り替えると、処理の違いを比較しやすくなります。要素数は通常 `n = 256` まで増やせ、たくさん並べると全体が整っていく様子をより迫力たっぷりに観察できます。ストゥージソートは `n = 48`、ビーズソートは `n = 64`、ボゴソートは `n = 7`、スローソートは `n = 40` が観察上限です。ビトニックソートでは要素数を2の累乗（最大256）から選びます。
