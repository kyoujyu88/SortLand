...
20種類のソートを、棒グラフとキャラクターで楽しく観察できるビジュアライザーです。

[デモを開く](https://kyoujyu88.github.io/SortLand/) · [搭載アルゴリズム](#搭載アルゴリズム) · [ローカル実行](#ローカル実行)

![Algorithms](https://img.shields.io/badge/algorithms-20-5fa9a3?style=flat-square)
...
アルゴリズムの説明を読むだけでなく、乱れた棒が少しずつ整っていく過程を実際に観察するための学習サイトです。それぞれのソートを女の子キャラクターとして表現し、特徴的な操作や考え方が視覚的に伝わるよう設計しています。

### 主な機能

- **20種類のソート** — 基本、高速、非比較、個性派の4カテゴリ
- **リアルタイム可視化** — 比較中・交換中・配置中の要素を色分け
- **自由な実験** — 要素数 `n` と再生速度をその場で変更
- **耳でも観察** — 値に応じた軽い木琴風の音と、完了時の上昇チャイム
...
> [!TIP]
> 同じ並びで別のアルゴリズムへ切り替えると、処理の違いを比較しやすくなります。ストゥージソートは非常に遅いため、観察上限を `n = 48` に設定しています。
...
| 高速 | Merge / Quick / Heap / Shell / Tim / Intro |
| 非比較 | Counting / Radix / Bucket |
| 個性派 | Gnome / Odd-Even / Cycle / Pancake / Strand / Stooge |
...
| `npm test` | 全20アルゴリズムの正当性と回帰テストを実行 |
...
キャラクターは全20人をSortLand専用に制作しています。各イラストの小物やポーズには、隣接交換、最小値選択、ピボット分割、ヒープ構造、桁分け、反転など、それぞれのアルゴリズム固有の考え方を反映しています。
